 ...
 */
export async function getCircleWallet(userId: string, role: 'manager' | 'tenant'): Promise<CircleWallet | null> {
  try {
    const response = await fetch(`${API_URL}/api/circle/wallet/${userId}?role=${role}`);
